...
//  m_networdShow->stop();
//  m_networdShow->setHidden(true);
  QString fileName = QFileDialog::getExistingDirectory(this,"Music","D:\\KuGou",QFileDialog::ReadOnly);
  if (fileName.isNull() || fileName.isEmpty())return;
  getAllFiles(fileName);
}

void MusicShow::on_loading_single()
{
  m_fileList->clear();
  m_player->stop();
  m_loading->setHidden(true);
  QLineEdit* inputWeb = new QLineEdit;
  inputWeb->setFixedWidth(120);
  m_layout->addWidget(inputWeb,2,0,1,1);
  connect(inputWeb,&QLineEdit::returnPressed,this,[=](){
      QString webAddr = inputWeb->text();
      on_loading_wed(webAddr);
      m_model->setStringList(QStringList(webAddr));
      m_loading->setHidden(false);
      inputWeb->deleteLater();
    });
}
...
// 文件加载
// 加载网络资源
void MusicShow::on_loading_wed(QString &address)
{
  m_fileList->clear();
  if (address.isEmpty())
    return;
  ...
  //m_networdShow->load(QUrl(netword));
}
// 窗口边框设置
